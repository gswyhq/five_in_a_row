...
def satrtGUI(ch):
    ...
    white = pygame.transform.smoothscale(white, (int(white.get_width() * 0.9), int(white.get_height() * 0.9)))
    black = pygame.transform.smoothscale(black, (int(black.get_width() * 0.9), int(black.get_height() * 0.9)))

    screen.blit(background, (0, 0))
    # font = pygame.font.SysFont("黑体", 40)
    # 查看pygame中可用的系统字体：
    # print(pygame.font.get_fonts())
    font = pygame.font.SysFont('wenquanyimicrohei', 40)
            ...
            if event.type == QUIT:
                ...
                if 25 <= x <= 725 and 25 <= y <= 725 and ((x - 25) % 50 <= level or (x - 25) % 50 >= 0) and (
                    ...
                    if Judge(m, n, color, ch.a, 4):
                        screen.blit(font.render('游戏结束，黑子赢了！', True, (110, 210, 30)), (80, 650))
                        break
                    ...
                    if Judge(x, y, color, ch.a, 4):
                        screen.blit(font.render('游戏结束，白子赢了！', True, (217, 20, 30)), (80, 650))
                        break
